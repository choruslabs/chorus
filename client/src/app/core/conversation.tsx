...
const CurrentComment = ({ comment = { content: 'test comment placeholder', id: 'placeholder' }, commentIndex = 0, onComplete }: { comment: Comment; commentIndex: number, onComplete: Function }) => {
  ...
  return (
    <div className='flex flex-col mx-auto w-full'>
      ...
      <p id="comment-description" className='text-gray-800 min-h-20'>{comment?.content}</p>
      <div id="comment-actions" className='flex w-full items-center gap-4 mb-4 flex-wrap'>
        <Button
...
const ConversationPage = () => {
  ...
  return (
    <CoreBase>
      <div id='converstion-container' className='flex flex-col h-full py-8 w-[95%] max-w-4xl mx-auto'>
        <h1 id='topic' className='text-3xl font-bold mb-4 text-gray-900'>{conversation?.name}</h1>
        <p id='description' className='mb-16'>{conversation?.description}</p>
