...
class TestCreateConversation:
    def test_create_conversation_success(
        self, db, authenticated_clients, valid_conversation_data
    ):
        """
    ...
    def test_create_conversation_missing_fields(self, authenticated_clients):
        ...
        """
        authenticated_client = authenticated_clients(1)["user1"]
        invalid_data = {"description": "This is a test conversation."}  # Missing 'name'

        response = authenticated_client.post("/conversations", json=invalid_data)
        assert response.status_code == 422

    def test_create_conversation_normalizes_link(
        self, db, authenticated_clients, valid_conversation_data
    ):
        """
        ...
        response = authenticated_client.post(
            "/conversations", json=valid_conversation_data
        )
        assert response.status_code == 200
    ...
    def test_create_conversation_no_link(
        self, authenticated_clients, valid_conversation_data
    ):
        """
...
class TestReadConversations:
    ...
    def test_read_conversations_returns_a_list(
    ...
    def test_read_conversations_schema(
        ...
        create_conversation(
            authenticated_client, {"name": "Schema Test Conversation 2"}
        )
...
class TestReadConversation:
    ...
    def test_read_conversation_not_found(self, authenticated_client):
        ...
        """
        response = authenticated_client.get(f"/conversations/{uuid4()}")
    ...
    def test_read_conversation_format(self, authenticated_clients, create_conversation):
        """
...
class TestUpdateConversation:
    def test_can_update_own_conversation(
        self, authenticated_client, create_conversation
    ):
        """
        ...
        conversation_id = create_conversation(
            authenticated_client, {"display_unmoderated": True}
        ).json()["id"]
    ...
    def test_cannot_update_anothers_conversation(
        self, create_conversation, authenticated_clients
    ):
        """
        ...
        conversation_id = create_conversation(
            conversation_owner, {"display_unmoderated": True}
        ).json()["id"]

        response = another_user.put(
            f"/conversations/{conversation_id}", json={"display_unmoderated": False}
        )
    ...
    def test_invalid_data(self, authenticated_client, create_conversation):
        ...
        response = authenticated_client.put(
            f"/conversations/{conversation_id}", json={"display_unmoderated": "hello"}
        )
        assert response.status_code == 422
    ...
    def test_update_conversation_normalizes_link(
        self, db, authenticated_client, create_conversation
    ):
        """
        Test link normalization when user provides an optional user-friendly link
        Expected Behavior:
    ...
    def test_update_conversation_allow_comments_and_votes(
        ...
        """
        conversation_id = create_conversation(
            authenticated_client, {"allow_comments": True, "allow_votes": True}
        ).json()["id"]
...
class TestDeleteConversation:
    def test_can_delete_own_conversation(
        self, authenticated_client, create_conversation
    ):
        """
    ...
    def test_can_delete_own_conversation_with_comments(
        self, authenticated_client, create_conversation
    ):
        """
        Test the successful deletion of a conversation by the user that created the conversation
    ...
    def test_cannot_delete_anothers_conversation(
        self, create_conversation, authenticated_clients
    ):
        """
    ...
    def test_delete_conversation_not_found(self, authenticated_client):
        ...
        - The API returns a 404 status code
        """
        response = authenticated_client.delete(f"/conversations/{uuid4()}")
        assert response.status_code == 404

    def test_delete_conversation_invalid_token(
        self, client, authenticated_client, create_conversation
    ):
        """
...
class TestGetConversationIdByFriendlyLink:
    def test_can_get_conversation_id_of_own_conversation(
        self, authenticated_client, create_conversation
    ):
        friendly_link = "link-to-conversation"
        conversation_id = create_conversation(
            authenticated_client, {"user_friendly_link": friendly_link}
        ).json()["id"]
    ...
    def test_can_get_conversation_id_of_anothers_conversation(
        self, authenticated_clients, create_conversation
    ):
        clients = authenticated_clients(2)
        conversation_owner = clients["user1"]
        ...
        conversation_id = create_conversation(
            conversation_owner, {"user_friendly_link": friendly_link}
        ).json()["id"]

        response = another_user.get(f"/conversations/friendly-link/{friendly_link}/id")
        assert response.status_code == 200
        assert response.json() == conversation_id

    def test_cannot_get_conversation_id_unauthenticated(
        self, client, authenticated_client, create_conversation
    ):
    ...
    def test_get_conversation_id_invalid_token(
        self, client, authenticated_client, create_conversation
    ):
        conversation_owner = authenticated_client
    ...
    def test_get_conversation_id_by_friendly_link_not_found(self, authenticated_client):
        response = authenticated_client.get(
            f"/conversations/friendly-link/nonexistent-link/id"
        )
        assert response.status_code == 404


class TestCreateComment:
    def test_conversation_owner_can_create_comment(
        self, db, authenticated_client, create_conversation
    ):
        conversation_id = create_conversation(authenticated_client).json()["id"]
        ...
        response = authenticated_client.post(
            f"/conversations/{conversation_id}/comments", json=comment_data
        )
        assert response.status_code == 200
        assert "id" in response.json()
    ...
    def test_non_owner_can_create_comment_on_conversation(
        self, db, create_conversation, authenticated_clients
    ):
        ...
        response_user2 = user2_client.post(
            f"/conversations/{conversation_id}/comments", json=comment_data_user2
        )
        assert response_user2.status_code == 200

        comment_data_user3 = {"content": "Comment from user 3"}
        response_user3 = user3_client.post(
            f"/conversations/{conversation_id}/comments", json=comment_data_user3
        )
        assert response_user3.status_code == 200

        # Query the database to check both comments were created
        comments = (
            db.query(Comment).filter_by(conversation_id=UUID(conversation_id)).all()
        )
        assert len(comments) == 2

    def test_cannot_create_comment_when_missing_fields(
        self, authenticated_clients, create_conversation
    ):
        authenticated_client = authenticated_clients(1)["user1"]
        ...
        response = authenticated_client.post(
            f"/conversations/{conversation_id}/comments", json=invalid_data
        )
        assert response.status_code == 422
    ...
    def test_create_commment_conversation_not_found(self, authenticated_client):
        ...
        response = authenticated_client.post(
            f"/conversations/{invalid_conversation_id}/comments", json=comment_data
        )
...
class TestReadComments:
    ...
    def test_invalid_include_user_info(self, authenticated_client, create_conversation):
        ...
        response = authenticated_client.get(
            f"/conversations/{conversation_id}/comments?include_user_info=invalid_value"
        )
        assert response.status_code == 422
    ...
    def test_read_comments_conversation_not_found(self, authenticated_client):
        ...
        response = authenticated_client.get(
            f"/conversations/{non_existent_conversation_id}/comments"
        )
        assert response.status_code == 404
    ...
    def test_read_only_approved_comments_when_display_unmoderated_is_false(
        ...
        conversation_id = create_conversation(
            conversation_owner, {"display_unmoderated": False}
        ).json()["id"]
        ...
        approved_comment_id = create_comment(
            user_2, conversation_id, approved_comment
        ).json()["id"]
        ...
        response = user_2.get(f"/conversations/{conversation_id}/comments")
        assert response.status_code == 200
        comments = response.json()
        assert len(comments) == 1
        assert comments[0]["content"] == approved_comment
    ...
    def test_can_see_and_vote_on_unmoderated_comments_when_display_unmoderated_is_true(
        ...
        unmoderated_comment_id = create_comment(
            user_2, conversation_id, unmoderated_comment_content
        ).json()["id"]
        ...
        response = user_3.post(
            f"/comments/{unmoderated_comment_id}/vote", json={"value": 1}
        )
        assert response.status_code == 200
        assert "id" in response.json()
...
class TestVoteOnComment:
    def test_can_vote_on_another_users_comment_first_time(
        ...
        # conversation owner and user 2 create a comment each on the conversation
        owner_comment_id = create_comment(conversation_owner, conversation_id).json()[
            "id"
        ]
        user2_comment_id = create_comment(user_2, conversation_id).json()["id"]

        # user 3 votes on user 2's comment
        response = user_3.post(f"/comments/{user2_comment_id}/vote", json={"value": 1})
        assert response.status_code == 200
        assert "id" in response.json()

        # user 3 votes on conversation owner's comment
        response = user_3.post(f"/comments/{owner_comment_id}/vote", json={"value": 1})
        assert response.status_code == 200
        assert "id" in response.json()

        # user 2 votes on conversation owner's comment
        response = user_2.post(f"/comments/{owner_comment_id}/vote", json={"value": 1})
        assert response.status_code == 200
        assert "id" in response.json()

        # conversation owner votes on user2's comment
        response = conversation_owner.post(
            f"/comments/{user2_comment_id}/vote", json={"value": 1}
        )
        assert response.status_code == 200
        assert "id" in response.json()
    ...
    def test_can_update_vote_on_another_users_comment(
        ...
        response = user_3.post(f"/comments/{comment_id}/vote", json={"value": 1})
        assert response.status_code == 200
    ...
    def test_can_vote_on_own_comment(
        self, authenticated_clients, create_conversation, create_comment
    ):
        clients = authenticated_clients(2)
        conversation_owner = clients["user1"]
        ...
        response = conversation_owner.post(
            f"/comments/{owner_comment_id}/vote", json={"value": 1}
        )
        assert response.status_code == 200
        assert "id" in response.json()

        # user 2 votes on their own comment
        response = user_2.post(f"/comments/{user2_comment_id}/vote", json={"value": 1})
        assert response.status_code == 200
        assert "id" in response.json()
    ...
    def test_vote_on_comment_not_found(self, authenticated_client):
        ...
        response = authenticated_client.post(
            f"/comments/{non_existent_comment_id}/vote", json={"value": 1}
        )

        assert response.status_code == 404
        assert response.json() == {"detail": "Comment not found"}

    def test_vote_on_comment_unauthenticated(
        self, create_conversation, create_comment, authenticated_client
    ):
        ...
        client.cookies.clear()
        response = client.post(f"/comments/{comment_id}/vote", json={"value": 1})

        assert response.status_code == 422  # should we get this to be 401 instead?

    def test_vote_on_comment_with_invalid_token(
        self, authenticated_client, create_conversation, create_comment
    ):
        conversation_id = create_conversation(authenticated_client).json()["id"]
        ...
        response = authenticated_client.post(
            f"/comments/{comment_id}/vote", json={"value": 1}
        )

        assert response.status_code == 401
        assert response.json()["detail"] == "Invalid token"

    def test_vote_on_comment_invalid_data(
        self, authenticated_client, create_conversation, create_comment
    ):
        conversation_id = create_conversation(authenticated_client).json()["id"]
        comment_id = create_comment(authenticated_client, conversation_id).json()["id"]
        ...
        response = authenticated_client.post(
            f"/comments/{comment_id}/vote", json={"value": "invalid_value"}
        )
...
class TestGetNextRemainingComment:
    def test_get_next_remaining_comment_success(
        ...
        comment_id = create_comment(
            user_3, conversation_id, "Comment made by someone out there"
        ).json()["id"]
        ...
        response = user_2.get(f"/conversations/{conversation_id}/comments/remaining")
        assert response.status_code == 200
    ...
    def test_get_next_remaining_comment_format(
        ...
        comment_id = create_comment(
            conversation_owner, conversation_id, "Comment"
        ).json()["id"]
        approve_response = approve_comment(conversation_owner, comment_id)
    ...
    def test_get_next_remaining_comment_no_comments(
        self, authenticated_client, create_conversation
    ):
        conversation_id = create_conversation(authenticated_client).json()["id"]
    ...
    def test_get_next_remaining_comment_skips_own_comments(
        ...
        user1_comment_id = create_comment(
            user_2, conversation_id, "User 2's comment"
        ).json()["id"]
    ...
    def test_get_next_remaining_comment_skips_voted_comments(
        ...
        comment_2_id = create_comment(
            user_2,
            conversation_id,
            "Another comment that is definitely different from the first",
        ).json()["id"]
        ...
        vote_response = vote_on_comment(
            user_3, first_remaining_comment_returned["id"], 1
        )
        ...
        assert (
            first_remaining_comment_returned["id"]
            != second_remaining_comment_returned["id"]
        )

        # User 3 votes on the second comment
        vote_response = vote_on_comment(
            user_3, second_remaining_comment_returned["id"], -1
        )
        assert vote_response.status_code == 200
    ...
    def test_get_next_remaining_comment_skips_rejected_comments(
        ...
        # user 2 creates two comments
        approved_comment_id = create_comment(
            user_2, conversation_id, "I will be approved."
        ).json()["id"]
        rejected_comment_id = create_comment(
            user_2, conversation_id, "I am a silly cat."
        ).json()["id"]
    ...
    def test_get_next_remaining_comment_conversation_not_found(
    ...
    def test_get_next_remaining_comment_unauthenticated(
        self, client, create_conversation
    ):
        conversation_id = create_conversation().json()["id"]

        client.cookies.clear()
        response = client.get(f"/conversations/{conversation_id}/comments/remaining")

        assert response.status_code == 422  # should we get this to be 401 instead?

    def test_get_next_remaining_comment_with_invalid_token(
        self, client, create_conversation
    ):
        conversation_id = create_conversation().json()["id"]
